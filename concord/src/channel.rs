	...
	/*
		...
		rustlet!("set_channel", {
			...
			ds_context.set_channel(channel_key, channel).map_err(|e| {
				let error: Error =
					ErrorKind::ApplicationError(format!("Error setting channel: {}", e.to_string()))
						.into();
				error
			})?;
		});
		...
		rustlet!("delete_channel", {
			...
			ds_context.delete_channel(channel_key).map_err(|e| {
				let error: Error =
					ErrorKind::ApplicationError(format!("Error deleting channel: {}", e.to_string()))
						.into();
				error
			})?;
		});
		...
		rustlet!("get_channels", {
			...
			let channels = ds_context
				.get_channels(server_pubkey, server_id)
				.map_err(|e| {
