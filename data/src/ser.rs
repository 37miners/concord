...
impl Readable for PublicKey {
	...
	fn read<R: Reader>(reader: &mut R) -> Result<Self, Error> {
		...
		let secp = Secp256k1::with_caps(ContextFlag::None);
		let pk = PublicKey::from_slice(&secp, &buf).map_err(|_| {
			let error: Error = ErrorKind::CorruptedData("invalid pub key".to_string()).into();
			error
		})?;
		Ok(pk)
	}
...
impl<T: Ord> VerifySortedAndUnique<T> for Vec<T> {
	fn verify_sorted_and_unique(&self) -> Result<(), Error> {
		for pair in self.windows(2) {
			if pair[0] > pair[1] {
				return Err(ErrorKind::SortError("sorting error".to_string()).into());
			} else if pair[0] == pair[1] {
				return Err(ErrorKind::DuplicateError("duplicate error".to_string()).into());
			}
		}
		Ok(())
	}
...
fn _serialize_error_kind<S>(kind: &io::ErrorKind, serializer: S) -> Result<S::Ok, S::Error>
where
	S: serde::Serializer,
{
	match *kind {
...
fn _deserialize_error_kind<'de, D>(deserializer: D) -> Result<io::ErrorKind, D::Error>
where
